...
export const getAllStudentNote = async (
  ...
  try {
    ...
    if (!page && !pageSize) {
      const getStudentNote = await StudentNote.findAll({where: whereClause});
...
export const updateNoticeById = async (noteId, teacherId, userType, newData) => {
  try {
...
export const deleteNoticeById = async function (noteId, teacherId, userType) {
  try {
    ...
    if (!deleteNotice) {
      return { success: false, message: "No Notice found with the provided ID" };
    }

    if (userType === "Teacher" && deleteNotice.teacherId !== teacherId) {
      return { success: false, message: "Unauthorized: You cannot delete this notice" };
    }
